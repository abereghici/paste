...
- Different from [notification/feedback messaging](/patterns/notifications-and-feedback), which is in direct response to a user action.
...
[Link-style buttons](/components/button/#examples) can be used when clicking the text performs an in-page action, such as opening a modal.
